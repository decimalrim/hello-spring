...
public class PaginateVO {
		 ...
		 * 이전 그룹의 시작 페이지 번호
		 */
		private int preGroupStartPageNo;
		...
		public void setPageCount(int listCount) {
			...
			// 전체 페이지 개수를 pageCountInGroup으로 나누었을 때 몇개의 페이지 그룹이 생길지 계산
			...
			// 현재 그룹에서 다음 그룹이 존재하는지 확인.
			this.hasNextGroup = this.groupNo + 1 < this.groupCount;
			
			// 현재 그룹에서 이전 그룹이 존재하는지 확인.
			this.hasPreGroup = this.groupNo > 0;
			
			//  다음 그룹의 시작 페이지 번호를 계산
			this.nextGroupStartPageNo = this.groupEndPageNo + 1;
			
			//  이전 그룹의 시작 페이지 번호를 계산
			this.preGroupStartPageNo = this.groupStartPageNo - this.pageCountInGroup;
		...
		public boolean isHasPreGroup() {
			return hasPreGroup;
		}

		public void setHasPreGroup(boolean hasPreGroup) {
			this.hasPreGroup = hasPreGroup;
		}
